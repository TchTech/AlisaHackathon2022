...
def main():
    ...
    if text:
        ...
        if len("".join(re.findall(r"\d\d\d|\d\d|\d", text))) > 0 and (users_states[user_id].say_weight == True):
            ...
            elif int(text) < 14 and int(text) > 0:
            ...
            ##Если пользователь ввёл отрицательный вес
                ...
                response_text = "Введённый вами имеет отрицательное значение!\nПопробуйте ещё раз."
                response_speak = response_text
                buttons = QuitAdviceButton
        ...
        if len("".join(re.findall(r"\d\d\d|\d\d", text))) > 0 and (users_states[user_id].say_growth == True):
            ...
            elif int(text) <= 67 and int(text) > 0:
        ...
        if ("расскажи" in text and "про" in text) or ("расскажи" in text and "о" in text) or ("расскажи" in text and "об" in text) or ("скажи" in text and "про" in text) or ("скажи" in text and "о" in text) or ("пожалуйста" in text and "про" in text) or ("что" in text or "как" in text and "насчёт" in text or "насчет" in text) or ("покажи" in text) or ("посчитай" in text and len(re.findall(r"\b\w*амм|\bг\w*|\bкг\w*", text)) <= 0): 
        ...
        if len(re.findall(r"п[оа][сз][чщ].?[ие][тд]а[йи]|р[оа][сз][сз]?ч[ие][тд]ай", text)) > 0 or "посчитай" in text or "рассчитай" in text:
            ...
            ##Если вес равен или меньше нуля
            if product == False:
        ...
        if len(re.findall(r"пом[оа][шщ]ь?", text)) > 0:
