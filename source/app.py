        ...
        if user_id not in users_first_command or users_first_command[user_id] == False:
            ...
            response = {
                "response": {
                    "text": response_text,
                    "end_session": end,
                    "buttons": UserFirstCommand, ##Кнопка которая и станет первой командой пользователя с подсчётом ценности продукта (например, чая)

                    ##Карточка - блок с картинкой, далее заголовком и текстом
                    "card": {
                        "type":"BigImage",
                        "image_id": "937455/e1833075f705a4649b2c",
                        "title":"Приветствуем тебя в \"ИнфоЕде\"!",
                        "description":"Навык \"ИнфоЕд\" успешно запущен, теперь ты можешь узнать пищевую ценность (белки, жиры, углеводы, калории) большинства продуктов!\nДля этого используй команду \"Посчитай\" и скажи название продукта.\nНапример: посчитай чай",
                    }
                },
                "version": version
            }
